...
def Show_GIF_Path(Agent_Set, result, save_path):
    plt.close('all')
    plt.figure(figsize=(12,12))
    ax = plt.gca()
    ax.cla()    
...
def RL_process(robot_num, eposide_num, epsilon, RL_SAVE_PATH):      
    for eposide in range(eposide_num):
        ...
        goal_flag = True
        for agent in Agent_Set:
            goal_flag = goal_flag and Check_Goal(agent, Calculate_distance(resX, resY, 0, 0), resTH)
        while(not goal_flag):
            if time > TIME_OUT:
                result = 'TIME_OUT'
                break
            ...
            Show_GIF_Path(Agent_Set, str(int(10*time)), GIF_path)
            time = time + deltaT
            goal_flag = True
            for agent in Agent_Set:
                goal_flag = goal_flag and Check_Goal(agent, Calculate_distance(resX, resY, 0, 0), resTH)
